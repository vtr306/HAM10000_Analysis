...
def correct_underexposure(im: np.ndarray, gamma: float, lambda_: float, kernel: np.ndarray, eps: float = 1e-3) -> np.ndarray:
...
def dual(im: np.ndarray, gamma: float = 0.6, lambda_: float = 0.15, sigma: int = 3,
                           bc: float = 1, bs: float = 1, be: float = 1, eps: float = 1e-3) -> np.ndarray:
    """Enhance input image, using DUAL method.
    ...
    # correct overexposure and merge
